...
export class CoursesController {
  ...
  @ApiOperation({
    ...
    description: 'Creates a new course with its lessons generated by OpenAI based on provided parameters.',
  })
  ...
  @ApiOperation({
    ...
    description: 'Retrieves the courses (with lessons) for the authenticated user.',
  })
  ...
  @Permissions()
  @ApiOperation({
    summary: 'Get completed user courses',
    description: 'Retrieves the completed courses (with lessons) for the authenticated user.',
  })
  @ApiResponse({ status: 200, description: 'Course data retrieved successfully.' })
  ...
  @ApiOperation({
    summary: 'Get user course',
    description: 'Retrieves the course (with lessons) for the authenticated user.',
  })
  @ApiResponse({ status: 200, description: 'Course data retrieved successfully.' })
  ...
  @ApiOperation({
    ...
    description: 'Retrieves all lessons associated with a given course for the authenticated user.',
  })
  ...
  @Permissions()
  @ApiOperation({
    summary: 'Answer lesson control question',
    description: 'Submits the student’s answer to a lesson’s control question and returns an evaluation.',
  })
  ...
  @ApiOperation({
    ...
    description: 'Generates a detailed interactive lesson content for a given lesson using OpenAI.',
  })
  @ApiResponse({ status: 200, description: 'Lesson content generated successfully.' })
  async createLessonContent(@UserId() userId: string, @Param('lessonId', ParseUUIDPipe) lessonId: string) {
    return await this.coursesService.createLessonContent(userId, lessonId);
  }

  @Post('/:courseId/cancel')
  @Permissions()
  @ApiOperation({ summary: 'Cancel user course' })
  @ApiResponse({ status: 200, description: 'Lesson content generated successfully.' })
  async cancelCourse(@Param('courseId', ParseUUIDPipe) courseId: string) {
    return await this.coursesService.cancelCourse(courseId);
  }
}
